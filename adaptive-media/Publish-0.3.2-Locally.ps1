...
try {
    ...
    $existing = Get-Release -ReleaseTag $Tag -AllowMissing
    if ($null -ne $existing) {
        $asset = Assert-032Release -Release $existing -ExpectDraft:$false
        Assert-Baseline031
    ...
    for ($i=0; $i -lt 5; $i++) {
        ...
        $draft = Get-Release -ReleaseTag $Tag -AllowMissing
        if ($null -ne $draft) {
    ...
    if ($CreatedDraft) {
        Write-Host 'Cleaning up the draft v0.3.2 release/tag created by this run...'
        try { Invoke-Gh -Arguments @('release','delete',$Tag,'--repo',$Repo,'--cleanup-tag','--yes') | Out-Null } catch { Write-Host ('Cleanup warning: ' + $_.Exception.Message) }
    }
