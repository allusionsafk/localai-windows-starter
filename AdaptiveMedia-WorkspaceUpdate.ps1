...
function Ensure-Using([string]$Path,[string[]]$Usings) {
    ...
    $text=[IO.File]::ReadAllText($Path); $prefix=''
    foreach($u in $Usings){ if($text -notmatch [regex]::Escape($u)){ $prefix += $u+[Environment]::NewLine } }
...
try {
    ...
    Write-Host ' Adaptive Media v0.3.0-dev7 - workspace repair + build' -ForegroundColor Cyan
    Write-Host '================================================================' -ForegroundColor Cyan
    Write-Host ''
    ...
    Say 'Existing source workspace found. Skipping the broken Base64 DevKit transport.'
    Say 'Applying WPF startup and build-gate migrations...'
    ...
    # App.xaml generates AdaptiveMedia.App. Do not declare it a second time.
...
'@
    ...
    Write-Utf8NoBom (Join-Path $AppDir 'Program.cs') @'
using System;
using System.IO;
using System.Linq;
...
internal static class Program
{
    ...
    public static int Main(string[] args)
    {
        if (args.Any(a => string.Equals(a, "--self-test", StringComparison.OrdinalIgnoreCase)))
        {
            ...
            return Environment.Is64BitOperatingSystem && File.Exists(engine) && File.Exists(config) ? 0 : 11;
        }
...
'@
    ...
    if($xml -match '<StartupObject>.*?</StartupObject>'){
        ...
        $xml=[regex]::Replace($xml,'(<PropertyGroup(?:\s+[^>]*)?>)','$1'+[Environment]::NewLine+'    <StartupObject>AdaptiveMedia.Program</StartupObject>',1)
    }
    ...
    $b=$b.Replace("if (`$Clean) { Remove-Item -Recurse -Force `$Build,`$Dist -ErrorAction SilentlyContinue }",
    ...
    $stagePattern="(?ms)Step 'Launcher self-test'\s*& \(Join-Path \$Stage 'AdaptiveMedia\.exe'\) --self-test\s*if \(\$LASTEXITCODE -ne 0\) \{ throw \"Native app self-test failed with exit code \$LASTEXITCODE\.\" \}\s*Write-Host 'Native app self-test: PASS' -ForegroundColor Green"
    $stageReplacement=@'
...
'@
    $b=[regex]::Replace($b,$stagePattern,$stageReplacement)

    $installedPattern='(?ms)^\s*& \$smokeLauncher --self-test\s*if \(\$LASTEXITCODE -ne 0\) \{ throw "Installed app self-test failed with exit code \$LASTEXITCODE\." \}'
...
'@
    $b=[regex]::Replace($b,$installedPattern,$installedReplacement)
    Write-Utf8NoBom $BuildScript $b

    $main=Join-Path $AppDir 'MainWindow.xaml'
    if(Test-Path $main){ $t=[IO.File]::ReadAllText($main); $t=[regex]::Replace($t,'v0\.3\.0-dev\d+','v0.3.0-dev7'); Write-Utf8NoBom $main $t }
    $iss=Join-Path $Source 'installer\AdaptiveMedia.iss'
    if(Test-Path $iss){ $t=[IO.File]::ReadAllText($iss); $t=[regex]::Replace($t,'0\.3\.0-dev\d+','0.3.0-dev7'); Write-Utf8NoBom $iss $t }

    if((Get-Content (Join-Path $AppDir 'App.xaml.cs') -Raw) -match 'class\s+App') { throw 'App.xaml.cs still declares App after migration.' }
    if((Get-Content (Join-Path $AppDir 'Program.cs') -Raw) -notmatch 'namespace AdaptiveMedia;') { throw 'Program.cs namespace migration failed.' }
    if((Get-Content $Proj -Raw) -notmatch '<StartupObject>AdaptiveMedia\.Program</StartupObject>') { throw 'StartupObject migration failed.' }
    Say 'Migration assertions: PASS'

    Say 'Building WPF app and installer...'
    & $BuildScript -InstallTools -Clean -SmokeTest *>&1 | Tee-Object -FilePath $Log -Append
    ...
    if(-not (Test-Path $launcher)){ throw "Built launcher not found: $launcher" }
    Say 'Running independent strict launcher self-test...'
    $p=Start-Process -FilePath $launcher -ArgumentList '--self-test' -Wait -PassThru
    if($p.ExitCode -ne 0){ throw "Independent launcher self-test failed with exit code $($p.ExitCode)." }
    Say 'Independent launcher self-test: PASS'

    $setup=Get-ChildItem (Join-Path $Source 'dist') -Filter 'AdaptiveMediaSetup-*.exe' -File | Sort-Object LastWriteTime -Descending | Select-Object -First 1
    if(-not $setup){ throw 'No installer EXE was produced.' }
    ...
    Write-Host ''
    Write-Host "This run DID update the log: $Log" -ForegroundColor Yellow
